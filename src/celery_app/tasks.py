...
def classify_image_task(self, image_data: str, task_id: str = None):
    ...
    try:
        ...
        if task_id:
            redis_client.setex(
                f"result:{task_id}", settings.REDIS_CACHE_TTL, json.dumps(result)
            )
...
def analyze_image_task(self, image_data: str, task_id: str = None):
    ...
    try:
        ...
        if task_id:
            redis_client.setex(
                f"result:{task_id}", settings.REDIS_CACHE_TTL, json.dumps(final_result)
            )

        return {"status": "SUCCESS", "result": final_result, "task_id": task_id}
...
def batch_process_task(
    self, images_data: List[Dict], task_type: str, task_id: str = None
):
    """Celery task for batch processing"""
    try:
        self.update_state(
            state="PROCESSING",
            meta={"status": f"Processing {len(images_data)} images..."},
        )
        ...
        for i, image_info in enumerate(images_data):
            try:
                ...
                self.update_state(
                    state="PROCESSING",
                    meta={
                        "status": f"Processing image {i + 1}/{total}",
                        "progress": progress,
                    },
                )
